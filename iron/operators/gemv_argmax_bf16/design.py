...
def my_matvec_argmax(
    ...
    dtype_in = np.dtype[bfloat16]  # bf16 inputs
    dtype_out = np.dtype[np.float32]  # f32 output (val + idx)
    L1_A_ty = np.ndarray[(m_input, K), dtype_in]
    L1_B_ty = np.ndarray[(K,), dtype_in]
    L1_C_ty = np.ndarray[(2,), dtype_out]  # f32 output
    # Running max buffer: persistent L1 memory, not ObjectFifo
    RunMax_ty = np.ndarray[(2,), dtype_out]  # [max_val, argmax_idx] f32
    L3_A_ty = np.ndarray[(M * K,), dtype_in]  # bf16 matrix
    L3_B_ty = np.ndarray[(K,), dtype_in]  # bf16 vector
    L3_C_ty = np.ndarray[(cols * 2,), dtype_out]  # f32 output

    matvec = Kernel(
        f"{func_prefix}matvec_argmax_bf16_bf16",
