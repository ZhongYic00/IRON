...
def rms_norm(x, w):
    ...
    return (xf * torch.rsqrt(xf.pow(2).mean() + eps) * w.to(torch.float32)).to(torch.float32)

def qk_norm(x, w):
    xf = x.float()
    var = xf.pow(2).mean(dim=-1, keepdim=True)
    return (xf * torch.rsqrt(var + eps) * w.float()).to(torch.float32)
...
def rope(xr, cos, sin):
    xf = xr.float()
    return (xf * cos + rotate_half(xf) * sin).to(torch.float32)
...
def main():
    ...
    emb = sf.get_tensor("model.embed_tokens.weight").to(torch.float32)
    ...
    for i in range(n_layers):
        Lw = f"model.layers.{i}"
        w_in_ln = sf.get_tensor(f"{Lw}.input_layernorm.weight").to(torch.float32)
        w_q = sf.get_tensor(f"{Lw}.self_attn.q_proj.weight").to(torch.float32)
        w_k = sf.get_tensor(f"{Lw}.self_attn.k_proj.weight").to(torch.float32)
        w_v = sf.get_tensor(f"{Lw}.self_attn.v_proj.weight").to(torch.float32)
        w_o = sf.get_tensor(f"{Lw}.self_attn.o_proj.weight").to(torch.float32)
        w_qn = sf.get_tensor(f"{Lw}.self_attn.q_norm.weight").to(torch.float32)
        w_kn = sf.get_tensor(f"{Lw}.self_attn.k_norm.weight").to(torch.float32)
        w_ln2 = sf.get_tensor(f"{Lw}.post_attention_layernorm.weight").to(torch.float32)
        w_g = sf.get_tensor(f"{Lw}.mlp.gate_proj.weight").to(torch.float32)
        w_u = sf.get_tensor(f"{Lw}.mlp.up_proj.weight").to(torch.float32)
        w_d = sf.get_tensor(f"{Lw}.mlp.down_proj.weight").to(torch.float32)

        xn = rms_norm(x, w_in_ln)  # (L, 1024)
        qkv = (torch.cat([w_q, w_k, w_v], 0).float() @ xn.float().T).T.to(torch.float32)  # (L, 4096)
        ...
        attn = torch.softmax(scores, dim=-1).to(torch.float32)  # (L,16,L)
        ctx = torch.einsum('bhm,mhd->bhd', attn.float(), v_all.float()).reshape(L, 2048).to(torch.float32)
        attn_out = (w_o.float() @ ctx.float().T).T.to(torch.float32)  # (L,1024)
        x = (x.float() + attn_out.float()).to(torch.float32)

        xn2 = rms_norm(x, w_ln2)
        gate = (w_g.float() @ xn2.float().T).T.to(torch.float32)
        up = (w_u.float() @ xn2.float().T).T.to(torch.float32)
        gate = torch.nn.functional.silu(gate.float()).to(torch.float32)
        ffn = (gate.float() * up.float()).to(torch.float32)
        ffn_out = (w_d.float() @ ffn.float().T).T.to(torch.float32)
        x = (x.float() + ffn_out.float()).to(torch.float32)
    ...
    # final norm cosine
    w_fin = sf.get_tensor("model.norm.weight").to(torch.float32)
